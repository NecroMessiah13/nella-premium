...
export default function Admin(){
 ...
 return <main className="adminPage">
  ...
  :tab==="collections"?<div className="adminGrid">
    ...
    <form className="adminForm" onSubmit={save}><h2>{edit?edit.id?"Редактирование коллекции":"Новая коллекция":"Выберите коллекцию"}</h2>{edit&&<><label>Название<input name="name" defaultValue={edit.name} required/></label><label>Slug<input name="slug" defaultValue={edit.slug} required/></label><label>Описание<textarea name="description" defaultValue={edit.description||""}/></label><label>Изображение<input name="image" defaultValue={edit.image||""} placeholder="https://...jpg"/></label>        <label>Порядок сортировки<input name="sortOrder" type="number" defaultValue={edit.sortOrder||0}/></label><label style={{display:'flex',alignItems:'center',gap:'8px',marginTop:'6px'}}><input type="checkbox" name="active" defaultChecked={edit.active!==false}/>Активна (показывать на сайте)</label><label style={{marginBottom:'12px'}}>Товары в коллекции ({selectedProducts.size} выбрано):</label><div style={{border:'1px solid var(--line)',borderRadius:'4px',maxHeight:'300px',overflowY:'auto',padding:'12px',background:'#fafaf9',marginBottom:'15px'}}>{products.length===0?<p style={{margin:0,color:'var(--muted)',fontSize:'12px'}}>Нет товаров</p>:products.map(p=><div key={p.id} style={{display:'flex',alignItems:'center',gap:'10px',padding:'8px',marginBottom:'6px',background:selectedProducts.has(p.id)?'#efe9e1':'#fff',borderRadius:'3px',border:selectedProducts.has(p.id)?'1px solid var(--ink)':'1px solid transparent',cursor:'pointer',transition:'.2s'}} onClick={()=>{const newSet=new Set(selectedProducts);if(newSet.has(p.id)){newSet.delete(p.id)}else{newSet.add(p.id)}setSelectedProducts(newSet);}}><div style={{width:'42px',height:'42px',flex:'0 0 auto',borderRadius:'3px',overflow:'hidden',background:'#e8e1d8',display:'grid',placeItems:'center',color:'#fff',fontSize:'10px',fontFamily:'Playfair Display, serif'}}>{p.images?.[0]?.url?<img src={p.images[0].url} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:"нет"}</div><input type="checkbox" checked={selectedProducts.has(p.id)} onChange={()=>{}} style={{marginRight:'2px',width:'16px',height:'16px',cursor:'pointer'}}/><div style={{flex:1,minWidth:0}}><div style={{fontSize:'13px',color:'#1c1b19',fontWeight:'500'}}>{p.name}</div><small style={{color:'var(--muted)',fontSize:'11px'}}>{p.color} · {money(p.price)}</small></div></div>)}</div><div className="formButtons"><button className="darkButton">Сохранить</button><button type="button" onClick={()=>{setEdit(null);setSelectedProducts(new Set());}}>Отмена</button></div>{msg&&<small style={{display:'block',marginTop:'10px'}}>{msg}</small>}</>}</form>
  </div>
